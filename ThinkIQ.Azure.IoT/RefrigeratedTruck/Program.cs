...
namespace refrigerated_truck
{
    class Program
    {
        ...
        const double
            tooWarmtooLong = 60; // Time in seconds for contents to start melting if temperatures are above threshold.
        ...
        // User IDs.
        static string IDScope = "0ne003A7E29";
        ...
        static async void SendTruckTelemetryAsync(Random rand, CancellationToken token)
        {
            while (true)
            {
                ...
                double truckContentTemperature = Math.Round(tempContents, 2);
                var telemetryDataPoint = new
                {
                    ContentsTemperature = truckContentTemperature,
                ...
                // Prepare component telemetry
                // Assume the component is 5 units warmer than the truck
                const double temperatureDiff = 5.0;
                var componentTelemetryDataPoint = new
                {
                    ContentsTemperature = Math.Round(truckContentTemperature + temperatureDiff, 2)  
                };
                var componentTelemetryMessageString = JsonSerializer.Serialize(componentTelemetryDataPoint);
                var componentTelemetryMessage = new Message(Encoding.ASCII.GetBytes(componentTelemetryMessageString))
                {
                    MessageId = Guid.NewGuid().ToString(),
                    CreationTimeUtc = creationTimeUtc,
                    ComponentName = "RefrigeratedTruck_3s9"     // Component name as defined in the DTDL model
                };
                ...
                await s_deviceClient.SendEventAsync(componentTelemetryMessage);
